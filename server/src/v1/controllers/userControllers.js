...
import { partnerService, staffService, userService } from "../../services/userServices.js";
...
const createNewUser = async (req, res) => {
    ...
    const { accessToken, refreshToken } = await sessionServices.createSession({
        userId: user.id,
        ipAddress: req.ip,
...
const onBoardPartner = async (req, res) => {
    ...
    const result = await partnerService.createPartner({
        userData: { fullName, password, email, dni, gymId, role: "partner", phone },
        profileData: { birthDate, height, weightHistory, observations },
        membershipData: { startDate, expirationDate, amount, planId, gymId: gymId },
    });
...
const getAllPartnersByGym = async (req, res) => {
    const partners = await partnerService.getAllPartnersByGym({ gymId: req.user.gymId });
